...
def load_mock_companies() -> pd.DataFrame:
    ...
    df = db_query("""
        SELECT
            ...
            cc.roa,
            CASE WHEN cc.cogs > 0 THEN cc.gross_profit / cc.cogs END AS berry_ratio,
    ...
    for i in range(150):
        ...
        companies.append({
            ...
            "berry_ratio": round(gross_profit / cogs if cogs > 0 else 1.05, 4),
...
def global_data_portal() -> None:
    ...
    # Disclaimer banner
    st.warning("⚠️ **数据声明 / Data Disclaimer:** 本数据库中的数据为演示用模拟数据，旨在展示系统功能和转让定价分析方法论。实际分析应使用经审计的真实财务数据。/ The data in this database is simulated for demonstration purposes only. Actual analysis should use audited real-world financial data.")
...
def comparability_wizard() -> None:
    ...
    if current_step == 0:
        ...
        st.markdown("""
        描述你要分析的关联交易，并完成功能分析(FAR：Functions-Assets-Risks)。
    ...
    if current_step == 1:
        ...
        st.markdown("""
        根据第1步的功能分析(FAR)结果，系统智能推荐最合适的转让定价方法。
    ...
    if current_step == 2:
        ...
        with st.expander("📊 定量筛选"):
            ...
            with qc3:
                rp_max = st.slider(
                    "关联交易占比上限 / Max Related-Party Ratio",
                    0.0, 1.0, 0.30,
                    format="%.0f%%",
                    help="💡 关联交易占比>30%的公司通常不适合作为独立可比对象。建议默认30%上限，确保可比公司以独立交易为主。依据：UN TP Manual Ch.3 可比性因素分析",
                )
        ...
        with st.expander("🔍 定性筛选"):
            ...
            with qual2:
                ...
                exclude_loss = st.checkbox(
                    ...
                    value=True,
                    help="💡 剔除营业利润率<0的公司。亏损公司通常不作为可比对象，因为亏损可能由非正常因素导致（如管理层决策失误、一次性减值），不具备可比性。依据：UN TP Manual Ch.3 / OECD TPG Ch.3",
                )
        ...
        if len(filtered) > 0:
            display_cols = ["company_name", "country", "industry", "revenue_musd",
                           "operating_margin", "berry_ratio", "roce", "related_party_pct", "fiscal_year"]
...
def country_policy() -> None:
    ...
    with check_col1:
        needs_master = mne_revenue >= 750
        icon_m = "✅" if needs_master else "❌"
        st.metric("主文件 / Master File", f"{'需要' if needs_master else '不需要'}", icon_m)
    with check_col2:
        needs_local = mne_revenue >= 50
        icon_l = "✅" if needs_local else "❌"
        st.metric("本地文件 / Local File", f"{'需要' if needs_local else '不需要'}", icon_l)
    with check_col3:
        needs_cbc = mne_revenue >= 750
        icon_c = "✅" if needs_cbc else "❌"
        st.metric("国别报告 / CbC Report", f"{'需要' if needs_cbc else '不需要'}", icon_c)

    if mne_revenue >= 750:
        st.success("📋 该企业需准备完整三层文档 / Full three-tier documentation required.")
        st.info("""
        **文档提交时间表 / Filing Timeline:**
        - 主文件 / Master File: 财年结束后12个月内 / Within 12 months of FY end
        - 本地文件 / Local File: 财年结束后12个月内 / Within 12 months of FY end
        - 国别报告 / CbC Report: 财年结束后12个月内 / Within 12 months of FY end
        """)
    elif mne_revenue >= 50:
...
def industry_benchmark() -> None:
    ...
    with pli_col2:
        st.markdown("##### Berry Ratio (毛利率/销售成本率)")
        if 'berry_ratio' in filtered.columns and filtered['berry_ratio'].notna().any():
            ...
            st.caption("💡 Berry Ratio适用于分销商：>1.0表示有毛利润，通常1.05-1.20为正常区间")
        else:
